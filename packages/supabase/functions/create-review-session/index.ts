...
const VERSION = "create-review-session@2026-02-18.1";
...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (honeypot_field) {
      ...
      return new Response(
        JSON.stringify({ session_id: "fake-id", session_token: "fake-token" }),
    ...
    return new Response(
      JSON.stringify({ session_id: session.id, session_token: rawToken }),
      { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  } catch (err) {
